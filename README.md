...
- A per-device half-duplex TX/RX lock, so hardware that can't receive and transmit simultaneously (e.g. CC1101-based RF front-ends) doesn't get corrupted commands -- see `backend/ir_rf_hub/esphome/device_session.py`
...
3. Flash `ir_rf_proxy` onto the ESPHome devices you want to use (see `DOCS.md` inside the app, or the App's own Documentation tab in Home Assistant) — this needs Home Assistant OS/Supervised, since Apps aren't available otherwise.
...
model, API surface) and `backend/tests/` / `frontend/` for how to run
things locally.
